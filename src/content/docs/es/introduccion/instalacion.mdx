...
# Instalación de Lenguaje Quetzal
...
1. **Descargar el ejecutable**
   ...
   ```
   https://github.com/AntaresGT/lenguaje-quetzal/releases
   ```
   
   Busca el archivo llamado `lenguaje-quetzal.exe` en la sección **Assets** de la release más reciente.
...
2. **Crear directorio de instalación**
   ...
   Mueve el archivo `lenguaje-quetzal.exe` descargado a esta carpeta.
...
```cmd
lenguaje-quetzal mi_programa.qz
```

### Modo interactivo

```cmd
lenguaje-quetzal --interactivo
```

### Ver ayuda

```cmd
lenguaje-quetzal --ayuda
```
...
<CardGrid>
  <Card title="Comando no reconocido" icon="error">
    **Error**: `'lenguaje-quetzal' no se reconoce como un comando interno o externo`
    ...
    **Solución**:
    ...
    - Asegúrate de que el archivo esté en `C:\quetzal\lenguaje-quetzal.exe`
  </Card>
...
dir C:\quetzal\lenguaje-quetzal.exe

# Verificar variables de entorno
echo %PATH%

# Ejecutar con información detallada
lenguaje-quetzal --version --verbose
```
...
1. Descarga la nueva versión de `lenguaje-quetzal.exe` desde GitHub Releases
2. Reemplaza el archivo anterior en `C:\quetzal\`
3. Verifica la nueva versión con `lenguaje-quetzal --version`
...
<CardGrid>
  ...
  <Card title="Sintaxis básica" icon="document">
    ...
    [Aprender sintaxis →](/es/fundamentos/sintaxis-basica)
  </Card>
